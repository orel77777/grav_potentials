...
def diff_norm_round_thor_potential(theta, r, x3, r0, R0):
    ...
    b = (((a/2)-(R1**2))+math.sqrt((((a/2)+(R1**2))**2) -
                                   ((4*(R1**2))*(((r)**2)/(R0**2)))))
    c = (((a/2)-(R1**2))-math.sqrt((((a/2)+(R1**2))**2) -
                                   ((4*(R1**2))*(((r)**2)/(R0**2)))))
    n = ((a-b)/(2*((r**2)/(R0**2))))
...
def norm_round_thor_potential(r, x3, r0, R0):
    '''
...
def norm_round_ring_potential(r, x3, R):
    ...
    '''
    phi = ((4*R)/(math.sqrt(((R+r)**2)+(x3**2)))) * \
        mpmath.ellipk(((4*R*r)/(((R+r)**2)+(x3**2))))
    return float(phi)
